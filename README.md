...
- 모든 노드를 탐색해 목적지까지 잇는 모든 거리의 총합이 최소가 되는 경로를 찾는다.
- 그래프 구조와 그리드 구조 둘 다 탐색 가능하다.
- 실제 경로 탐색에서 사용할 그래프는 경로점들이 있고 이 경로점들의 길이가 cost가 된다.
- 가는 경로에 장애물이 있다면 cost를 무한대로 넣어주고 그 길을 피하게 만든다.

- IoT 제어에서는 그리드맵을 사용하여 다익스트라를 사용한다.

#### Occupancy Grid Map

**격자로 연속적인 공간을 표현하는 방법**

- occupied space : 물체가 존재하는 공간
- free space : 물체가 존재하지 않는 공간
- unknown space : 아직 탐색 되지 않은 공간
- 연속적인 공간에서는 좌표를 사용, 그리드 맵에서는 각 칸의 번호를 사용한다.
- 공간을 칸으로 표현하면 칸이 채워져있는지 안채워져있는지 다루기 편하다.
  - ex) 연속공간에서 좌표를 사용하면 0 < x < 1, 0 < y < 1인 곳을 확인해야하지만 그리드 맵에서는 칸 번호의 데이터만 확인하면 된다. 즉, 연속 공간을 대표하는 칸을 만들어서 사용한다.
- Occupancy가 붙으면 각 칸의 데이터 값이 채워져있는 공간, 채워져있지않는 공간, 모르는 공간을 의미한다.

**맵 파라미터를 이용해 공간간 좌표변환을 할 수 있다.**

- resolution : 그리드 한 칸 당 연속공간에서의 크기
- size x : x축 방향으로 맵의 크기
- size y : y축 방향으로 맵의 크기
- origin x : 연속공간에서의 맵의 시작 위치 x
- origin y : 연속공간에서의 맵의 시작 위치 y

- 연속공간의 좌표 ↔ 그리드 공간 좌표를 알아야 한다.

#### Cost Map

**충돌확률을 고려한 맵**

- 주행 로봇의 크기를 고려해 비용 함수 정의
- 충돌 확률에 따라 각 cell의 값이 정해진다.

- Occupancy Grid map은 그곳에 무엇이 있는지 없는지 나타낸 맵이다.

- 이 맵을 기반으로 경로를 탐색하게 되면 최단 경로이기 때문에 occupied space 바로 옆의 free space를 선택해서 경로를 찾게 된다.

- 이 경로는 로봇이 지나갈 수 없다 → 로봇의 크기가 고려되어 있지 않기 때문에 벽에 부딪친다.

- Occupancy Grid map을 이용해서 로봇의 크기를 고려하여 충동 확률를 나타낸 것이 Cost Map이다.
  - ex) 로봇의 크기가 1m이라고 생각하면 Occupied space 중심으로 1m안으로 있는 칸들은 충돌이 무조건 일어나는 구간, 1 ~ 1.5m는 충돌이 일어날 수도 있는 구간, 1.5m보다 큰 곳은 충돌이 일어나지 않는 구간으로 정한다.

- 이렇게 정한 것을 비용함수라고 하고 비용함수를 적용한 맵이 Cost Map이다.

- 충돌회피를 하기 위해서는 2가지의 Cost Map을 만들어야 한다. 두가지를 이용하면 충돌없이 목적지까지 갈 수 있다.
  - Global Cost Map
    - 가지고 있는 맵의 비용함수를 이용해 초기 한번 제작한다.
    - 맵이 만들어진 시점의 정보만 가지고 있다.
    - 정적장애물에 대한 충돌만 고려된 맵이다.
    - 전역경로를 만들 때 사용한다.
  - Local Cost Map
    - 라이다 센싱 공간에 대해 그리드 맵을 만들고 라이다 포인트를 비용함수에 적용하여 제작한다.
    - 라이다 데이터가 들어 올 때마다 업데이트한다.
    - 동적, 정적, 장애물에 모두 고려된 맵이다.

#### Global Path Planning

**Global Cost Map을 이용하여 전역 경로를 획득하는 방법**

1. 로봇의 공간 좌표(x, y)를 Global Cost Map에 대한 grid 좌표로 변환한다.
   - 로봇의 공간 좌표는 연속된 좌표이기 때문에 공간좌표로 변환
2. 목적지 grid좌표를 입력한다.
3. Dijkstra 알고리즘을 이용해 최단 경로를 얻는다.
4. 탐색한 경로는 grid 좌표로 얻기 때문에 우리가 사용하고 있는 공간 좌표(x, y)로 변환한다.

**Local Cost Map을 이용하여 지역 경로를 획득하는 방법**

1. 전역경로를 Local Cost Map에 대한 grid 좌표로 변환한다.
2. 변환한 경로점들의 cost를 확인해서 충돌 여부를 판단한다.
3. 충돌시 Dijkstra를 이용해 경로를 탐색한다.
4. 탐색한 경로는 grid 좌표로 얻기 때문에 우리가 사용하고 있는 공간 좌표(x, y)로 변환한다.

**정리**

전역경로는 처음에 전역경로 계획을 통해 한번만 만들어주고 전역 경로를 따라가는 도중에 충돌체크를 계속 한다.

충돌 시에는 Local cost Map에 최단 경로 알고리즘을 이용하여 장애물을 회피하는 경로를 다시 만들어주고 그것을 따라가게 되면 목적지까지 충돌을 회피하며 갈 수 있다.

### Lattice planner

**격자(Lattice)를 이용해 충돌을 회피하는 방법**

- 이 방법은 가지고 있는 경로를 이용하여 후보 경로를 미리 만들면서 주행을 하다가, 가장 안전하다고 생각하는 경로를 선택하여 주행하는 방법이다.

- 이 방법은 후보 경로를 만드는 과정, 후보 경로를 선택하는 과정 두 단계로 이루어져 있다.

- 기존 경로(Reference Path)를 이용해 다양한 조건으로 후보 경로를 만든다.

- 후보 경로 중 안전(Safety), 부드러움(Smoothness), 일관성(Consistency) 등을 고려해 주행할 경로를 선택한다.

## 센서 기반 주행 방법

### Wall follwing(실습 사용)

**거리 센서(초음파, 라이다, 적외선)를 이용해 벽과 일정 거리를 유지하며 주행하는 로봇**

- 설정한 거리보다 클 경우, 왼쪽 바퀴를 더 빠르게 돌린다.
- 설정한 거리보다 작을 경우, 오른쪽 바퀴를 더 빠르게 돌린다.
- 벽이 하나만 있을 경우 180도 이상 돌아가지 않는 이상 주행을 할 수 있다.
- 거리에 따라 비례하는 회전 속도를 다르게 줘야한다. → 적절한 값 필요
- 비례하는 값은 P, PI, PID 제어를 통해 찾는다.

**Wall Following은 주변 환경에 따라 다양한 상황을 고려해야 한다.**

- 양쪽 벽의 거리를 측정해야 한다.
- 각속도뿐만 아니라 선속도도 제어할 필요가 있다.
- 긴급상황에 따라 정지 또는 후진도 고려해야 한다.
- 알고리즘이 정해져 있는 것이 아니라 주변 환경과 가지고 있는 센서에 따라 데이터를 잘 분석해서 주행할 선속도와 각속도를 조절해서 충돌없이 벽을 잘 따라갈 수 있게 설계해야한다.
